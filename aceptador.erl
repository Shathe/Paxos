...
-export([aceptadorUnaInstancia/6, aceptador/3]).
...
aceptador(Yo, Servidores, ListaAceptadores)->
	...
	receive
		...
		{prepara, N, NodoFrom, NumInstancia} -> 
			...
			true -> %Sino existía el proceso, crearlo, guardarlo, reenviarlo
				Proc=spawn_link(Yo, ?MODULE, aceptadorUnaInstancia, [Yo,Servidores, NumInstancia, -1, -1, -1]),
				...
				spawn_link(Yo, ?MODULE, comprobar_estado_paxos, [Proc]),
				aceptador(Yo,Servidores, NuevaListaAceptadores)
			end;
		...
		Mensaje   -> io:format("Mensaje en proponente no identificado ~p, ~p ~n",[Yo, Mensaje]),
			     aceptador(Yo, Servidores, ListaAceptadores)
	end.

 
%%Proceso que gestiona el aceptador de una unica instnacia
aceptadorUnaInstancia(Yo,Servidores, NumeroInstancia, N_p, N_a, V_a )->
	...
	receive
		...
		{prepara, N, NodoFrom, NumInstancia} -> 
			...
			true->%Gestionar mensaje prepara
			    ...
			    if
				N > N_p -> %Enviar acepta
				    {paxos, NodoFrom} ! {prepara_ok, N, N_a, V_a, Yo, NumInstancia},
				    aceptadorUnaInstancia(Yo, Servidores, NumeroInstancia, N, N_a, V_a);
				true -> % Enviar el reject
				    {paxos, NodoFrom} ! {prepara_reject, N_p, Yo, NumInstancia},
				    aceptadorUnaInstancia(Yo,Servidores, NumeroInstancia,  N_p, N_a, V_a)
			    end
			end;
		...
		{acepta, {NumInstancia, Valor}, N, NodoFrom} -> 
			...
			N >= N_p->%Gestionar mensaje acepta
				N_pp = N, N_aa = N, V_aa = {NumInstancia, Valor},
				{paxos, NodoFrom} !  {acepta_ok, N, Yo, NumInstancia},
				aceptadorUnaInstancia(Yo, Servidores, NumeroInstancia, N_pp, N_aa, V_aa);
			true ->
				{paxos, NodoFrom} ! {acepta_reject, N_p, Yo, NumInstancia},
				aceptadorUnaInstancia(Yo, Servidores, NumeroInstancia, N_p, N_a, V_a)
			end;
		...
		comprobar_estado -> %Comprueba el estado de los valores de otros nodos de paxos, por si vas retrasado
			...
			io:format("Comprobando el valor de la instancia ~p en el nodo ~p~n",[NumeroInstancia, Yo]),
			if ExisteValor ->
				{paxos, Yo} ! {decidido, {NumeroInstancia, Valor}},
				io:format("Se actualiza el valor de la instancia ~p en el nodo ~p~n",[NumeroInstancia, Yo]);
			true-> sigo_esperando
			end,
			aceptadorUnaInstancia(Yo, Servidores, NumeroInstancia, N_p, N_a, V_a);
		...
		Mensaje   ->  io:format("Mensaje en aceptador no identificado ~p, ~p ~n",[Yo, Mensaje]),
					aceptadorUnaInstancia(Yo, Servidores, NumeroInstancia, N_p, N_a, V_a)
	end.
